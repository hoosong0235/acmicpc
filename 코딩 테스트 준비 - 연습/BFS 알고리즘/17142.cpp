...
int n, m, min_num = INT_MAX, ztotal = 0;
int a[MAX][MAX];
...
int calc() {
    ...
    while (!q.empty()) {
        ...
        if (ztotal == zcnt) break;
    }
    
    for (int i = 0; i < n; i++) for (int j = 0; j < n; j++) if (!visited[i][j] && a[i][j] != 1 && a[i][j] != 2) return INT_MAX;
    return c.d + 1;
}
...
int main() {
    ...
    for (int i = 0; i < n; i++) for (int j = 0; j < n; j++) {
        cin >> a[i][j];
        if (a[i][j] == 0) ztotal++;
        else if (a[i][j] == 2) cs.push_back({i, j});
    }
